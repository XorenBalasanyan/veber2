...
class GdThumb extends ThumbBase
{
	 ...
	 * @return GdThumb
	 */
	public function adaptiveResizeFixQuadrant ($width, $height, $quadrant = 'C')
	{
		// make sure our arguments are valid
		if (!is_numeric($width) || $width  == 0)
		{
		...
		// возврашаем объявленные размеры
		$this->maxHeight	= intval($height);
		$this->maxWidth		= intval($width);		
		...
		if ($this->currentDimensions['width'] < $this->maxWidth)
		{
		    ...
		    switch ($quadrant) {
		        ...
		        case 'R':
		            $posX = intval(($this->maxWidth - $this->currentDimensions['width']));
		            break;

		        case 'C':
		        default:
		            $posX = intval(($this->maxWidth - $this->currentDimensions['width']) / 2);
		            break;
		    }


		}
		if ($this->currentDimensions['height'] < $this->maxHeight)
		{
		    // Image is portrait
			switch ($quadrant) {
		        case 'T':
		        ...
		        case 'B':
		            $posY = intval(($this->maxHeight - $this->currentDimensions['height']));
		            break;

		        case 'C':
		        default:
		            $posY = intval(($this->maxHeight - $this->currentDimensions['height']) / 2);
		            break;
		    }

		}
		
		ImageCopy($this->workingImage, $this->oldImage, $posX, $posY, 0, 0, $this->currentDimensions['width'], $this->currentDimensions['height']);
		// update all the variables and resources to be correct
		$this->oldImage 					= $this->workingImage;
		$this->currentDimensions['width'] 	= $this->maxWidth;
		$this->currentDimensions['height'] 	= $this->maxHeight;
